...
class BaseUnit(ABC):
    ...
    @property
    def health_points(self) -> float:  # TODO возвращаем аттрибут hp в красивом виде
        return round(self.hp, 1)

    @property
    def stamina_points(self) -> float:  # TODO возвращаем аттрибут stamina в красивом виде
        return round(self.stamina, 1)

    def equip_weapon(self, weapon: Weapon) -> str:   # TODO присваиваем нашему герою новое оружие
        self.weapon = weapon
        return f"{self.name} экипирован оружием {self.weapon.name}"

    def equip_armor(self, armor: Armor) -> str:    # TODO одеваем новую броню
        self.armor = armor
    ...
    def use_skill(self, target: BaseUnit) -> str:
        ...
        if self._is_skill_used:
            return f"{self.unit_class.skill} был уже использован"
...
class EnemyUnit(BaseUnit):
    ...
    def hit(self, target: BaseUnit) -> str:
        ...
        if self._is_skill_used and self.stamina > self.unit_class.stamina and randint(0, 100) < 10:
